...
import { title_plural, title_singular, link } from "./_constant";
...
export default async function Page({ searchParams: search }: pageProps) {
  ...
  const { niches, categories, brands } = await getCatalogFilterOptions();
  ...
  return (
    <ResourcePage
      ...
      filters={<TableAction niches={niches} categories={categories} brands={brands} />}
    >
      <Suspense
        key={key}
